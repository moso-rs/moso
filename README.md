...
![MSRV 1.90](https://img.shields.io/badge/MSRV-1.90-000000?logo=rust&logoColor=white)
...
**Status: unreleased.** The workspace is 20 crates at `0.1.0` and nothing is published. The HTTP
...
```sh
git clone https://github.com/moso-rs/moso
cd moso
cargo check --workspace --all-features     # cold build, and it should be clean
cargo run -p moso-cli -- new shop          # scaffold an application
```
...
Rust ≥ 1.90 stable — `rust-toolchain.toml` selects it and installs `rustfmt` and `clippy`, so there
