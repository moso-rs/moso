...
// `chrono`'s date/time types validate trivially too, but only compile in when the
// `chrono` feature does — they are `Schema` scalars gated on the same feature.
#[cfg(feature = "chrono")]
trivial_validate!(DateTime<Utc>, NaiveDate, NaiveTime, NaiveDateTime);
...
anonymous_schema!(Uuid, "Uuid", true, |_g| StringBuilder::new()
    .format("uuid")
    .build());
// The `chrono` date/time scalars, behind the `chrono` feature. The base tree
// carries no date library: a string field with `#[schema(format = "date-time")]`
// is validated by `checks::is_valid_format` (via `time`) with no `chrono` in the
// graph. A database-backed build turns the feature on through `moso-orm`.
#[cfg(feature = "chrono")]
