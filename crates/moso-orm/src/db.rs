...
compile_error!(
    "moso-orm needs at least one backend: enable the `postgres` feature, the `sqlite` feature, \
     or both (both are on by default)"
);
...
impl Db {
    ...
    pub fn stats(&self) -> PoolStats {
        let (size, idle) = match &self.inner.pool {
            #[cfg(feature = "postgres")]
    ...
    pub(crate) async fn acquire(&self) -> Result<PooledConnection> {
        ...
        let acquired = match &self.inner.pool {
            #[cfg(feature = "postgres")]
