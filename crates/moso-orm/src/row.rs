...
macro_rules! try_get {
    ($row:expr, $index:expr, $ty:ty) => {
        match &$row.repr {
            ...
            RowRepr::Unbacked(never) => match *never {},
        }
    };
}
...
impl Row {
    ...
    pub fn get_uuid(&self, index: usize) -> Result<uuid::Uuid, DecodeError> {
        ...
        // Bound with a type so a backend-less build can infer the error type of
        // the macro's `Result`; there the macro expands to only a diverging arm.
        let native: core::result::Result<uuid::Uuid, sqlx::Error> =
            try_get!(self, index, uuid::Uuid);
    ...
    pub fn get_decimal(&self, index: usize) -> Result<moso_sql::Decimal, DecodeError> {
        ...
        // The annotation pins the type: in a backend-less build only the
        // diverging `Unbacked` arm remains, so nothing else fixes it to `String`.
        let text: String = match &self.repr {
            #[cfg(feature = "postgres")]
