...
";
    ...
    fn the_real_manifest_can_be_bumped() {
        ...
        // Derive the current version from the manifest rather than hardcoding it,
        // so the test does not rot every time the workspace version changes.
        let workspace = Workspace::load().expect("the workspace loads");
        let current = workspace
            .package("moso")
            .expect("the facade crate exists")
            .version
            .clone();
        let next = format!("{current}-bump.test");
        let (rewritten, changes) = bump_manifest(&text, &current, &next).expect("valid");
        assert!(
            changes.len() >= 2,
            "the workspace version and at least one pin: {changes:?}"
        );
        assert!(rewritten.contains(&format!("version = \"{next}\"")));
