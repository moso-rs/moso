...
pub struct Options {
    ...
    /// Report rule 6 (the crate-count budget) without failing the gate on it;
    /// the layering rules 1-5 stay fatal. CI sets this because the budget is a
    /// deliberately-open known-red (AGENTS.md), the same treatment `expand-size`
    /// (G14) gets. A local `cargo xt check-deps` leaves it unset and stays red.
    pub budget_advisory: bool,
}
...
pub fn run(options: &Options) -> Result<bool> {
    ...
    // Rules 1-5 are the layering invariants and always fail the gate. Rule 6 is
    // the crate-count budget: a deliberately-open known-red (AGENTS.md) that CI
    // reports without enforcing, exactly as `expand-size` (G14) is.
    let budget_over = report
        .rules
        .iter()
        .any(|rule| rule.id == 6 && !rule.status.ok());
    if options.budget_advisory && budget_over {
        ui::warn(
            "rule 6 (crate-count budget) is over budget - reported, not enforced \
             (known red; AGENTS.md forbids closing it by raising the number)",
        );
    }
    let ok = report
        .rules
        .iter()
        .all(|rule| rule.status.ok() || (rule.id == 6 && options.budget_advisory));
    Ok(ok)
}
