...
struct DepsArgs {
    ...
    #[arg(long)]
    json: Option<PathBuf>,
    /// Report the crate-count budget (rule 6) without failing on it; the
    /// layering rules 1-5 stay fatal. CI passes this so the known-red budget is
    /// reported like `expand-size`. Omit it locally to keep seeing the red.
    #[arg(long)]
    budget_advisory: bool,
}
...
fn dispatch(command: &Command) -> Result<bool> {
    match command {
        ...
        Command::CheckDeps(args) => deps::run(&deps::Options {
            ...
            json: args.json.clone(),
            budget_advisory: args.budget_advisory,
        }),
