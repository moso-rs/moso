...
| G10 | Dependency count within budget (90 default / 260 full) | merge (advisory) | `xtask check-deps --budget-advisory` |
...
**G10 and G14 are reported, not enforced.** The third-party crate count (151 default / 295 full)
and the `#[endpoint]` expansion size are both over budget today, and [AGENTS.md](../../AGENTS.md)
forbids closing either by raising the number. CI runs `check-deps --budget-advisory` - which keeps
the layering rules (G9) fully merge-blocking while letting only the count through as a warning - and
runs `expand-size` under `continue-on-error`, so both stay visible in the log without failing the
build. A local `cargo xt check-deps` leaves the flag off and still shows G10 red.
